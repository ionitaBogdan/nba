...
function renderTop5(players) {
  ...
  players.forEach((p, index) => {
    ...
    li.innerHTML = `
      ...
      <span class="steals">${p.steals} steals</span>`;
...
async function updateTop5(season) {
  ...
  const valid = allData.filter(p => typeof p.steals === 'number' && !isNaN(p.steals));
  const sorted = valid.sort((a, b) => b.steals - a.steals).slice(0, 10);
  renderTop5(sorted);
}
